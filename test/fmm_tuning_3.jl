...
function fmm_cost_error(wing, expansion_order, multipole_threshold, leaf_size, ϕ_direct, v_direct)
    ...
    # fmm calculation
    tree = fmm.Tree(wing; leaf_size, expansion_order, shrink_recenter=true)
    switch = fmm.DerivativesSwitch(true, false, true, false, wing)
    farfield, nearfield, self_induced = true, true, true
    m2l_list, direct_list = fmm.build_interaction_lists(tree.branches, tree.branches, multipole_threshold, farfield, nearfield, self_induced)
    direct_list = fmm.InteractionList(direct_list, wing, tree, wing, tree, switch)
    t_fmm = @elapsed fmm.fmm!(tree, wing, m2l_list, direct_list, switch)

    # extract potential and velocity
...
function get_results(;
        ...
        leaf_sizes = [25,50,100,200,400,800],
...
function plot_results(results_bson_prefix::String, ms; contour_type="abs", nc0=5, ns0=50)
    ...
    for m in ms
        ...
        for (i_p,expansion_order) in enumerate(expansion_orders)
            ...
            fig.tight_layout()
            fig.savefig("/Users/ryan/Dropbox/research/notebooks/img/20240621_fmm_tuning_wing3/fmm_cost_n$(n)_p$(expansion_order).png")
            ...
            fig.savefig("/Users/ryan/Dropbox/research/notebooks/img/20240621_fmm_tuning_wing3/fmm_cost_error_contour_" * contour_type * "_n$(n)_p$(expansion_order).png")
        end # p
    end # m
end
...
results_bson = "fmm_tuning_3"
ms = 1:8
# results = get_results(; ms, file=results_bson)
...
# m=3
# BSON.@load results_bson*"_m$m.bson" result n expansion_orders leaf_sizes multipole_thresholds
# εs_ϕ, εs_v_abs, εs_v_rel, t_direct, ts_fmm, ϕ_direct, v_direct, ϕs_fmm, vs_fmm = result
