...
function solve_gmres!(y::AbstractVector,
    ...
    if isnothing(Avalue)        # Case: No primal values provided
        ...
        if eltype(A) <: FD.Dual || eltype(A) <: RD.TrackedReal ||
           eltype(b) <: FD.Dual || eltype(b) <: RD.TrackedReal

            # Solve the linear system through GMRES while preserving AD support.
            y .= IAD.implicit_linear(A, b; lsolve=this_gmres, Af=Avalue)

        else

            # In the non-AD case, use the extracted primal values directly.
            y .= this_gmres(Avalue, b)
