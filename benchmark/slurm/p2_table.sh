...
#SBATCH --mem=500G
#SBATCH --requeue
# partition/qos default changed 2026-08-24: m12/normal left the
# 08-24 jobs PENDING on Priority indefinitely; physics2/standby has
# 10 idle-ish nodes and started them within seconds. The three
# partitions are spec-identical (2x64 zen3, 512 GB), so this does not
# affect comparability. Safe with --requeue because every driver
# resumes per row. Override back with --partition/--qos if needed.
#SBATCH --time=24:00:00
#SBATCH --output=slurm-%x-%j.out
#SBATCH --error=slurm-%x-%j.err

# 021 Phase 2a componentized setup + per-step cost table for one rung + mode.
# PREPARED 2026-08-18, NOT SUBMITTED — submission waits on the Phase 1 freeze
# + Ryan. Requires the rung's Phase-1 tuning CSVs (tune.csv, fgstune_*,
# fgsprecond.csv) under results/phase1/multi/<rung>/.
# Submit from the top level of the FLOWPanel.jl checkout with the campaign's
# pinned hardware (decision_rules timing protocol, 2026-08-18):
#   sbatch --job-name=p2-table-R4-multi \
#     --export=ALL,RUNG=R4,MODE=multi,CONFIGS=backslash_ldiv:krylov_gmres:krylov_jacobi:krylov_ilu:fgs:fgmres_fgs \
#     benchmark/slurm/p2_table.sh
# MODE=single runs the 1-thread/BLAS-1 mode (ruling 6). Cost-ceiling
# drop-outs (ledger): backslash_ldiv out at R6-R7 both modes; krylov_gmres
# out of single mode at R6-R7. --mem is NOT a per-rung knob any more: the job
# takes the whole 512 GiB zen3 node (--exclusive --mem=0), which covers the
# R5 dense G (~94 GiB) outright.
set -euo pipefail
...
echo "021 Phase 2a table"
echo "  repo:    $(pwd)"
echo "  rung:    $RUNG  mode: $MODE  threads: $THREADS  k_reps: $K_REPS"
echo "  configs: $CONFIGS"

julia --project=. --startup-file=no --compiled-modules=existing -t "$THREADS" benchmark/rotor_hover_solver_phase2.jl

echo "phase2 table complete for $RUNG/$MODE"
