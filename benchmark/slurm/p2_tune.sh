...
# 021 Phase 2 apply-knob tuning against a REAL SOLVE, over the machine-class
# memory ladder (rotor_hover_solver_phase2_tune.jl). Phase 2 OWNS tuning since
# the 2026-08-24 descope of Phase 1.
#
# ONE JOB PER RUNG, ALL FOUR BUDGETS SEQUENTIAL (Ryan's ruling 2026-08-24:
# "every job writes its own CSV; synthesize afterwards"). PER_RUNG_DIR gives
# each rung its own tune_phase2.csv, so one job per rung is exactly one writer
# per file — splitting the budgets across concurrent jobs would put four
# writers on one NFS file, the append hazard that destroyed R1's rows on
# 2026-08-18. Budget 0 must come first (phase2.jl reads it), which the natural
# ordering of the list already gives.
#
#   sbatch --job-name=p2-tune-R1 --time=12:00:00 \
#     --export=ALL,RUNG=R1,MEM_BUDGETS=0:16:128:500 benchmark/slurm/p2_tune.sh
#
# Long walltimes are cheap now: the driver checkpoints EVERY tuning candidate
# to tune_trace_<rung>_b<budget>.csv and replays it into the tuner's memo on
# restart, so a standby preemption costs one candidate, not a whole descent.
# Pair with --requeue (already in the header above).
#
# MEM_BUDGETS is TOTAL memory in GiB (body + solver state + FMM plan + cache):
#   0    cache FORBIDDEN - the matrix-free endpoint. REQUIRED at every rung:
#        it is what tunes the uncached configs, and at R1-R5 no positive
#        budget ever returns an uncached winner. RUN THIS FIRST.
#   16   laptop        128  workstation        500  supercomputer node
#
# The cache BUILD is excluded from the tuning objective by construction (an
# untimed warm-up solve builds it; only warm solves are timed) and recorded in
# t_cache_build. Ryan's ruling, 2026-08-24.
#
# Do NOT override --mem/--constraint/--exclusive: pinned hardware. Override
...
# All 13 jobs of the 2026-08-24 Phase 1+2 launch stalled ~12 h inside package
# loading, before one line of driver code ran. Julia's auto-precompilation
# takes a per-package pidfile lock in the NFS depot; job 13469151 (phys-1-7)
# won the lock and held it, and the other seven jobs blocked on it. Job
# 13469154's .err recorded the diagnosis verbatim:
#     Being precompiled by another machine (hostname: phys-1-7, pid: 2905490)
# with a final wait of 4.32e7 ms = 12.0 h. The same log shows an EACCES writing
# the BangBangDataFramesExt pidfile and a stale pidfile for Meshes, so the NFS
# lock path is itself unreliable here. Cost: p2-tune-R1/R2 and p1-agree-R1/R2
# burned their entire 12 h walltime and produced nothing.
# p2b_nearfield.sh already carried this guard; this wrapper never got it.
#
# THIS ONLY DISABLES THE IMPLICIT PER-JOB ATTEMPT — it does not build the
# cache. The serialized precompile block immediately below now does that
# automatically, so no manual pre-step is required before a fan-out.
...
: "${MEM_BUDGETS:?set MEM_BUDGETS via --export=ALL,MEM_BUDGETS=0}"
export MEM_BUDGETS
# Only export TUNE_REPS when actually set — unset means the driver's per-rung
# default (5 at R1-R4, 2 at R5+) applies. Forcing 5 here would more than double
# every R5+ descent. Note TUNE_REPS is part of the tuning trace's provenance
# guard, so it must not change between a run and its resume.
if [[ -n "${TUNE_REPS:-}" ]]; then export TUNE_REPS; fi
export NFCACHE_MAX_BUILD_TIME="${NFCACHE_MAX_BUILD_TIME:-Inf}"
# Only export TUNE_MAX_SECONDS when actually set: the driver parses it as a
# Float64, and an empty string would abort the job at startup. Unset means the
# driver's per-rung default table applies.
if [[ -n "${TUNE_MAX_SECONDS:-}" ]]; then export TUNE_MAX_SECONDS; fi

echo "021 Phase 2 real-solve tuning"
echo "  repo:     $(pwd)"
echo "  rung:     $RUNG"
echo "  budgets:  $MEM_BUDGETS (GiB TOTAL memory)"
echo "  threads:  $THREADS"


# ---- Load-time observability (added 2026-08-25) ----------------------------
# The 12 h precompile stall above was INVISIBLE: the .out held only bash's
# 290-byte banner for the whole run. Julia block-buffers stdout when it is a
# file, so no driver println ever reached disk, and sacct TotalCPU is not
# gathered on this cluster (a COMPLETED job reports 00:00:00), so neither
# source could distinguish "stuck" from "working". Three cheap fixes:
#   JULIA_DEBUG=loading  names each package as it loads, on unbuffered stderr,
#                        so a load-time stall identifies the package it is on.
#   heartbeat            a timestamped line every 5 min, so "alive but stuck"
#                        is distinguishable from "gone" and the stall window
#                        is bracketed in wall-clock time.
#   start/end stamps     bracket the julia invocation itself.
export JULIA_DEBUG="${JULIA_DEBUG:-loading}"

( while true; do
    echo "[$(date -u +%Y-%m-%dT%H:%M:%SZ)] heartbeat: job alive"
    sleep 300
  done ) &
HEARTBEAT_PID=$!
trap 'kill "$HEARTBEAT_PID" 2>/dev/null || true' EXIT
echo "[$(date -u +%Y-%m-%dT%H:%M:%SZ)] launching julia"
# ---------------------------------------------------------------------------

status=0
julia --project=. --startup-file=no --compiled-modules=existing -t "$THREADS" benchmark/rotor_hover_solver_phase2_tune.jl || status=$?
echo "[$(date -u +%Y-%m-%dT%H:%M:%SZ)] julia exited with status $status"
exit "$status"
