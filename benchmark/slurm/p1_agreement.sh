...
# 021 Phase 1 (DESCOPED 2026-08-24): solve the ladder up to R7 and verify that
# every solver agrees on the SOLUTION. Tuning lives in Phase 2 now; wall clock
# recorded here is incidental and must NOT be quoted as a benchmark.
#
#   sbatch --job-name=p1-agree-R7 --time=3-00:00:00 \
#     --export=ALL,RUNG=R7,CONFIGS=krylov_gmres:krylov_ilu:fgs:fgmres_fgs \
#     benchmark/slurm/p1_agreement.sh
#
# krylov_jacobi IS NOT SUPPORTED AT R7 (Ryan, 2026-08-25). Job 13469158 spent
# 9 h 20 min on it after krylov_gmres had finished the same rung in 7315 s /
# 86 iters, and was cancelled. Jacobi stays selectable at R1-R6, where it
# converges in reasonable time; at R7 always pass CONFIGS explicitly. Note the
# COLON separator - a comma inside --export=ALL,... breaks sbatch's own parsing
# and would silently select NOTHING (driver splits on [:,], see :310-314).
#
# ERROR METRIC IS THE BOUNDARY CONDITION, NOT A REFERENCE. Each config is
# judged by bc_rel_l2 on its OWN solution, evaluated through the FMM at
# 0.1 x target_rel x rms_b (an order of magnitude tighter than the criterion)
# and reported with the FMM's own bc_certified flag. Consistency is then
# measured against the ENSEMBLE (agreement_spread.csv). There is NO privileged
# reference and config ORDER DOES NOT MATTER, which is what makes R6/R7
# possible at all - a dense backslash_ref would be 335 GiB at R6 and 1.31 TiB
# at R7.
#
# CONFIGS subsets the roster. Drop backslash_ref/backslash_coupled above R4:
# they assemble a dense 8N^2 G (25 GiB at R4, 87 at R5, 335 at R6, 1.3 TiB at
# R7). krylov_ilu's pattern is ~23 GiB of entries at R7.
# NFCACHE=<GiB> optionally caches the Krylov configs purely for speed; it moves
# no reported metric and is skipped with a notice when it does not fit.
# RESIDUAL_BACKEND=direct gives the exact O(N^2) BC evaluation - only tractable
# at R1-R2, worth running there once as a cross-check of the FMM metric.
#
# Do NOT override --mem/--constraint/--exclusive: pinned hardware.
set -euo pipefail
...
export NFCACHE="${NFCACHE:-0}"
export RESIDUAL_BACKEND="${RESIDUAL_BACKEND:-fmm}"
# one writer per file: the 7 rung jobs run concurrently on different nodes and
# this driver holds agreement.csv open for hours before writing. A shared NFS
# append is what destroyed R1's rows on 2026-08-18. Merge with p021_merge.jl.
export PER_RUNG_DIR=1

echo "021 Phase 1 agreement (BC metric, reference-free)"
echo "  repo:     $(pwd)"
echo "  rung:     $RUNG"
echo "  configs:  ${CONFIGS:-<all>}"
echo "  nfcache:  $NFCACHE GiB"
echo "  residual: $RESIDUAL_BACKEND"
...
status=0
julia --project=. --startup-file=no --compiled-modules=existing -t "$THREADS" benchmark/rotor_hover_solver_phase1_agreement.jl || status=$?
