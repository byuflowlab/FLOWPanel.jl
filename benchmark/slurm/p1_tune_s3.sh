...
# DEPRECATED 2026-08-22 — use `p1_tune.sh` with STAGES=3 instead. This fork
# predates the BRAINSTORM/025 filament-regularization pin (2026-08-20) and so
# silently ran with the new Gaussian default; the pin is added below, but any
# future stage split should go through p1_tune.sh's STAGES gate rather than
# forking this file again.
#
# 021 Phase 1 tune pipeline STAGE 3 ONLY (fgsprecond sweep ladder) — recovery
# launcher for a chain whose tune_fmm + fgstune stages already completed
# (their CSVs present in results/phase1/multi/<RUNG>/). Same env contract as
# p1_tune.sh. Submit from the top level of the FLOWPanel.jl checkout.
set -euo pipefail
...
echo "021 Phase 1 tune pipeline (stage 3 only)"
echo "  repo:    $(pwd)"
echo "  rung:    $RUNG"
...
echo "tune stage-3 pipeline complete for $RUNG"
