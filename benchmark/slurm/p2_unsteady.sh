...
# 021 unsteady full-timestep-share run (ruling 7) for one rung + config +
# threading mode; Phase 3 warmstart runs reuse the same driver at longer
# schedules (ruling 12). PREPARED 2026-08-18, NOT SUBMITTED — waits on the
# Phase 1 freeze + Ryan. One config per job (the wake state is per-run).
# Submit from the top level of the FLOWPanel.jl checkout, pinned hardware:
#   sbatch --job-name=p2-unsteady-R4-fgs \
#     --export=ALL,RUNG=R4,MODE=multi,CONFIG=fgs,N_STEPS=72 \
#     benchmark/slurm/p2_unsteady.sh
# Leave N_STEPS unset for the full RHPC staged-startup + settle schedule
# (Phase 3 wake-developed regime); Phase 2's share metric needs only a short
# run (e.g. N_STEPS=72 = 2 revs at NT=36).
set -euo pipefail
...
echo "021 unsteady (Phase 2 share / Phase 3 warmstart)"
echo "  repo:   $(pwd)"
echo "  rung:   $RUNG  mode: $MODE  threads: $THREADS  config: $CONFIG"
echo "  steps:  ${N_STEPS:-<full RHPC schedule>}"

julia --project=. --startup-file=no --compiled-modules=existing -t "$THREADS" benchmark/rotor_hover_solver_unsteady.jl

echo "unsteady complete for $RUNG/$MODE/$CONFIG"
