...
# 021 Phase 2b near-field influence-matrix cache, one rung: isolated + solve
# A/B (cached vs kernel near field) and the cached-economics tune_fmm stage
# (TUNE_CACHED -> tune_cached.csv, consumed by the *_nfcache configs of
# rotor_hover_solver_phase2.jl). Submit from the top level of the
# FLOWPanel.jl checkout on the campaign's pinned hardware:
#   sbatch --job-name=p2b-nearfield-R1 \
#     --export=ALL,RUNG=R1,MODE=multi,TUNE_MACS=0.3:0.4:0.5:0.6:0.7:0.8 \
#     benchmark/slurm/p2b_nearfield.sh
# Requires the rung's Phase-1 knob CSVs + bcache under
# results/phase1/multi/<rung>/ (KNOBS_MODE=multi).
#
# Near-field cache caps (2026-08-22): the driver now defaults to 32 GiB /
# 1800 s, raised from 4 GiB / 60 s after the 08-20 campaign died on them —
# R4 needed 4.47 GiB (12% over) and R3 tripped the tuner's first-trial guard.
# The cache build is SERIAL, so the time cap is wall-clock, not per-thread.
# Override per rung with TUNE_MAX_BYTES / TUNE_MAX_BUILD_TIME via --export.
# Memory is no longer a per-rung knob: the job takes the whole 512 GiB
# zen3 node (--exclusive --mem=0), which clears the 32 GiB cap outright.
set -euo pipefail
...
# p2b checkout: PythonPlot/CondaPkg state is not synced (benchmark path never
# plots); skip env auto-precompile so unrelated precompile failures cannot
# abort the job — FLOWPanel/FastMultipole caches are already built.
# GeoIO is NOT a dependency any more: it was dropped from Project.toml on both
# checkouts 2026-08-22 to close the .CondaPkg/lock EACCES class, and
# examples/dji9443_trailing_edge.jl now reads meshes through the native
# pnl.read_gmsh. Leaving the cluster examples/ tree unsynced after that removal
# is what killed all ten 2026-08-22 jobs at load time (see
# BRAINSTORM/021.../phase_12_relaunch_prompt.md).
...
TUNE_MACS="${TUNE_MACS:-0.3:0.4:0.5:0.6:0.7:0.8}"
...
export TUNE_CACHED=1
export TUNE_MACS
export HARDWARE_TAG="${HARDWARE_TAG:-zen3-exclusive}"

echo "021 Phase 2b near-field cache A/B + cached tune"
echo "  repo:      $(pwd)"
echo "  rung:      $RUNG  mode: $MODE  threads: $THREADS"
echo "  tune_macs: $TUNE_MACS"

julia --project=. --startup-file=no --compiled-modules=existing -t "$THREADS" benchmark/rotor_hover_solver_phase2b_nearfield_cache.jl

echo "phase2b nearfield complete for $RUNG/$MODE"
