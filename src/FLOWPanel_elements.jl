...
NOTE: `offset` must be no smaller than 1e-10 or you may encounter some random
...
"""
Computes the velocity induced by a 2D panel of vertices `nodes[:, panel]` and
linear source strength (varying linearly between `sigma1` and `sigma2`) on the
targets `targets`. It adds the velocity at the i-th target to out[i].

Implementation of equations in Katz and Plotkin's Secs. 10.2.1 and 10.3.1.
...
function U_2D_linear_source(nodes::Arr1, panel,
    ...
    for p3 in 1:nt
        ...
        # Velocity components
        # Notice that the velocity induced by the source panel is
        # usource = -vvortex and vsource = uvortex
        u = (sigma1 + dsigma*(x - x1)/d)*logr1r2 - dsigma + dsigma*y/d*(theta2 - theta1)
        u /= 2*pi

        v = (sigma1 + dsigma*(x - x1)/d)*(theta2 - theta1) - dsigma*y/d*logr1r2
        v /= 2*pi
