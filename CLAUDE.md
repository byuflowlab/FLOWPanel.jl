...
- Keeping HPC disk under the 400 G cap / archiving finished runs → `hpc-storage` (tars finished runs to `/nobackup/archive` and only then strips their ParaView output; sweeps live runs' VTK as a fallback; never edits the protect list or touches the queue)
