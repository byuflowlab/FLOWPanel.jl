...
run_name        = "duct_remeshed_refined"             # Name of this run
...
if run_name == "duct_remeshed"
    meshfile          = joinpath(read_path, "duct_ovsp.msh")    # Gmsh file to read 
elseif run_name == "duct_remeshed_thicker"
    meshfile          = joinpath(read_path, "duct_remeshed_thicker.msh")    # Gmsh file to read 
elseif run_name == "duct_remeshed_refined"
    meshfile          = joinpath(read_path, "duct_remeshed_refined.msh")    # Gmsh file to read 
elseif run_name == "duct_thicker_thicker"
    meshfile          = joinpath(read_path, "duct_thicker_thicker.msh")    # meshfile          = joinpath(read_path, "duct_thicker_thicker_refined.msh")    # Gmsh file to read 
else
    error("Unsupported run name")
end
...
Sref            = pi * diameter * duct_length                      # (m^2) reference area
...
trailingedge[1, :] .= TE_x 
thetas = range(0, stop=2*pi+2*pi/nte, length=nte)
trailingedge[2, :] .= 0.5*diameter * cos.(thetas)
trailingedge[3, :] .= 0.5*diameter * sin.(thetas)
...
# shedding = hcat(shedding[:,1:21], shedding[:, 23:23]) # Only keep the first shedding points for debugging

# Freestream vector
Vinf = magVinf*[cos(AOA*pi/180), 0, sin(AOA*pi/180)]

# Freestream at every control point
Uinfs = repeat(Vinf, 1, grid.ncells)
...
UDeltaGamma = pnl.calcfield_Ugradmu(body; Gammai=2, force_cellTE=false)

# Add both velocities together
pnl.addfields(body, "Ugradmu", "U")

# Calculate gauge pressure (based on U + U_∇μ)
@time Ps = pnl.calcfield_P(body, magVinf, rho; correct_kuttacondition=false)
...
# Force coefficients
nondim = 0.5*rho*magVinf^2* (pi * diameter * duct_length)   # Normalization factor
