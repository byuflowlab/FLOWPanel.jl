...
function make_single_triangle_body()
    ...
    # create a decagonal fan (center + 10 boundary vertices) -> 10 triangles
    cx, cy = 0.5, 0.5
    r = 0.45
    n = 10
    vertices = [Point(cx, cy, 0.0)]
    for i in 0:n-1
        θ = 2π * i / n
        push!(vertices, Point(cx + r*cos(θ), cy + r*sin(θ), -0.2))
    end

    boundary = 2:length(vertices)
    triangles = [
        connect((1, boundary[k], boundary[mod1(k+1, length(boundary))]))
