...
| `026_sigma_growth_particle_splitting/particle_splitting_design.md` | Particle splitting to cap σ growth: growth-side triggers plus isotropic (viscous) and anisotropic (rVPM-compression) child geometries, extending FLOWVPM's existing shrink-side splitting; motivated by the 018 NT144 FMM cliff (one runaway-σ particle) and the gpu40 stretching ignition. §8 band-aid σ ceiling deployed separately. | [ ] | [ ] | [ ] | DESIGN 2026-08-27, revised 2026-09-02 (review): §3a algebra corrected, M[7] attribution replaced, warm-start `SplittingState` restore identified as Phase-0 BLOCKER (§9 gate cannot fire without it); Ryan purpose ruling: anisotropic split aims to conserve overlap + add a strain dof for stability, test paired with a σ floor → item RESET BRIEF. |
| `027_vpm_presentation_figures.md` | Concept figures (TikZ) for Ryan's how-VPM-works presentation: particle/regularization, operator splitting, growing list; Opus subagents generate, Ryan approves each. Presentation support — tangential to the CT mission. | [ ] | [ ] | [ ] | Opened 2026-08-29; 5 figures (F1 composite, F3a–d) built through 3 feedback rounds, F3c geometry Ryan-approved, rest awaiting approval; next = internet supplement images → item RESET BRIEF. |
| `028_nonuniform_filter_width_sfs_error.md` | Quantify non-uniform particle/filter-width error in the SFS model, distinguish commutation from reconstruction error, and evaluate conservative mitigation or explicit-filter decoupling. | [ ] | [ ] | [ ] | STAGED 2026-08-31 — theory and measurement plan only. Next: audit actual filter/core semantics and freeze manufactured commutator tests → item RESET BRIEF. |
| `029_projected_vorticity_basis_convergence.md` | Extension of 010: determine convergence of normalized scalar-blob vorticity versus the divergence-free vorticity implied by its Biot–Savart velocity, then decide whether the basis should be retained, reinterpreted, or replaced. | [ ] | [ ] | [ ] | STAGED 2026-08-31 — convergence and basis-decision plan only. Next: freeze domain, zero-mode, norm, and coupled h–σ contracts → item RESET BRIEF. |
